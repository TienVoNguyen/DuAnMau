...
//                                            frmQR.setSize(900, 900);
    ...
    private void mnuQRActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_mnuQRActionPerformed
        // TODO add your handling code here:
        MyMess.msgTrue(this, "Chúng tôi đang tạo một QR Code\n Vui lòng đợi...!");
        String maQR = "";
        try {
//            Kiểm tra đã tạo bao nhiêu mã
            String sql = "select count(ngaytao) SLTao from QRCODE where (DATEDIFF(day, NGAYTAO, GETDATE())) = 0 and MANV = ? group by NGAYTAO";
            ResultSet result = APIHelper.query(sql, Auth.nguoiDungHienTai.getMaNV());
            if (result.next()) {
                if (result.getInt("SLTao") >= 3) {
                    MyMess.msgFalse(this, "Mỗi ngày bạn chỉ được tạo 3 mã QR\n Ngày mai hãy quay lại");
                    return;
                }
            }
            //        tạo qr code
            maQR = XQRCode.writeQRCode();
            if (maQR.equalsIgnoreCase("")) {
                MyMess.msgFalse(this, "Tạo mã QR thất bại");
                return;
            }
            MyQRCode qr = new MyQRCode(maQR, Auth.nguoiDungHienTai.getMaNV(), DateHelper.toString(new Date(), "MM/dd/yyyy"));
            dao.insert(qr);
        } catch (Exception ex) {
            MyMess.msgFalse(this, "Tạo mã QR thất bại");
            ex.printStackTrace();
...
//        Gửi mail
        ...
        try {
            ...
            MyMess.msgTrue(this, "Chúng tôi đã gửi 1 mã QR Code có thời hạn 3 ngày vào email của bạn \n "
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        ...
        try {
            ...
            Logger.getLogger(QRCodeFrm.class.getName()).log(Level.SEVERE, null, ex);
        }
